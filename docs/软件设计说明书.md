...
1. 抽象和逐步求精的原则：将软件系统分解为多个抽象的模块，每个模块负责特定的功能，通过逐步求精的方式，将模块的复杂度降低，提高模块的可维护性和可扩展性。
2. 模块化与高内聚度、低耦合度的原则：将软件系统分解为多个独立的模块，每个模块负责特定的功能，模块之间的通过定义良好的接口进行通信，以实现模块之间的解耦合和可维护性。
3. 信息隐藏的原则：将软件系统的实现细节封装在模块内部，只暴露必要的接口，以保护软件系统的内部状态和行为。
4. 多视点以及关注点分离的原则：将软件系统的不同功能模块（如用户界面、业务逻辑、数据访问等）分离，每个模块负责特定的功能，以提高模块的可维护性和可扩展性。
5. 软件重用的原则：将已有的软件组件、库、框架等重用，以减少开发成本和时间，提高软件系统的可维护性和可扩展性。
6. 迭代设计的原则：采用迭代开发方法，将软件系统的开发过程分解为多个迭代周期，每个迭代周期负责实现软件系统的某个功能模块，以提高开发效率和软件系统的可维护性和可扩展性。
7. 可追踪性的原则：在软件系统的开发过程中，记录每个迭代周期的开发进度、代码变更、功能实现等信息，以方便后续的维护和扩展。
8. 离线优先的原则：保证客户端在弱网/无网环境下仍可正常使用基本功能，恢复联网后自动同步
...
### 3.4.5 精化 LoginPage 类属性的设计

在用户界面设计中，有 `LoginPage` 界面用于支持用户输入邮箱和密码登录到系统之中，具体见图 3-6 所示。`LoginPage` 界面类属性的精化设计描述如下：
...
### 3.4.7 精化"用户服务"子系统中部分类方法的设计

图 3-21 描述了"用户服务"子系统中实现用户登录功能的部分类及其设计。对于 `LoginPage` 界面类而言，根据其职责它具有两个 `public` 方法：`onSubmit()` 和 `onCancel()`，分别实现登录和取消的功能。此外，为了在登录之前检查输入的合法性，它有两个 `private` 方法：`isEmailValid()` 和 `isPasswordValid()`，分别用于判断用户输入的邮箱和密码是否满足相关的规范和要求。
...
<p align="center">图 3-21 精化设计 LoginPage、AuthService、UserRepository 等类的方法</p>
...
如果某个类在实现其职责过程中需要执行一系列的方法、与其他对象进行诸多的交互，那么可以考虑构造和绘制针对该类某些职责的活动图。图 3-25 用 UML 的活动图及泳道机制描述了 `LoginPage`、`AuthService`、`UserRepository` 三个类对象之间如何通过交互和协作来实现用户登录的功能。
...
<p align="center">图 3-25 LoginPage、AuthService、UserRepository 协作完成用户登录的活动图</p>
